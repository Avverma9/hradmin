...
export const fetchFilteredBookings = createAsyncThunk('booking/fetchFilteredBookings', async (queryParams, { rejectWithValue }) => {
    try {
        const response = await axios.get(`${localUrl}/get/all/filtered/booking/by/query?${queryParams}`, {
            headers: {
                Authorization: token,
            },
        });
...
export const createBooking = createAsyncThunk('booking/createBooking', async ({ userData, bookingData }, { rejectWithValue }) => {
    try {
        const response = await axios.post(`${localUrl}/booking/${userData?.userId}/${userData?.hotelId}`, bookingData, {
            headers: {
                Authorization: token,
            },
        });
