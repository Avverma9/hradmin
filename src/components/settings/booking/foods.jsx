...
export default function Food({ foodData, onFoodSelect }) {
  const handleSelect = (foodItem) => {
    ...
    onFoodSelect(foodItem);  // Assuming onFoodSelect is passed from parent component
  };

  return (
    <Box sx={{ padding: 1 }}>
      {/* Check if foodData is an array or a single object */}
      {Array.isArray(foodData) && foodData.length === 0 ? (
        <Typography variant="h6">No food items available</Typography>
      ) : (
        <Grid container spacing={1}>
          {/* If foodData is an array, map over it, otherwise directly display one food item */}
          {(Array.isArray(foodData) ? foodData : [foodData]).map((foodItem, index) => (
