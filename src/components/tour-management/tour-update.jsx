...
import { Save, Edit, Close, CheckCircle, Cancel, ArrowBack, Add, PhotoCamera } from "@mui/icons-material";
...
export default function TourUpdate() {
 ...
 const handleCountryChange = (event) => {
    ...
    handleInputChange('country', countryCode); // ISO code now
...
const handleStateChange = (event) => {
    ...
    handleInputChange('state', stateCode); // ISO code now
    handleInputChange('city', '');
};

const handleCityChange = (event) => {
    handleInputChange('city', event.target.value); // Name is fine
};
  ...
  <Container maxWidth="xl" sx={{ py: 3, bgcolor: 'grey.50', minHeight: '100vh' }}>
   ...
   <Grid container spacing={3}>
    ...
    {cities.map(city => (
       ...
       <Grid item xs={12} sm={6}><TextField fullWidth size="small" label="Themes" value={editableData.themes || ''} onChange={e => handleInputChange('themes', e.target.value)} disabled={!editSections.basic} /></Grid>
