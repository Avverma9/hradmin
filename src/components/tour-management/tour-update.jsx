...
const SectionPaper = ({
  ...
  <Paper
    variant="outlined"
    sx={{ p: { xs: 2, sm: 2.5 }, borderRadius: 3, mb: 2.5 }}
  >
    <Box
      display="flex"
...
export default function TourUpdate() {
  ...
  const handleSaveSection = async (sectionKey) => {
    ...
    try {
      ...
      toast.success(
        `${sectionKey.charAt(0).toUpperCase() + sectionKey.slice(1)} updated successfully!`
      );
  ...
  const handleAcceptanceChange = async (isAccepted) => {
    ...
    try {
      ...
      toast.success(
        `Tour status updated to ${isAccepted ? "Accepted" : "Declined"}.`
      );
      dispatch(tourById(id));
    } catch (err) {
  ...
  if (error) {
    return (
      <Box sx={{ p: 4 }}>
        <Alert severity="error">Error loading tour details.</Alert>
      </Box>
    );
  }
  ...
  if (!editableData) {
    return (
      <Box sx={{ p: 4 }}>
        <Alert severity="warning">Tour data not found.</Alert>
      </Box>
    );
  }
  ...
  return (
    <Container
      maxWidth="xl"
      sx={{ py: 3, bgcolor: "grey.50", minHeight: "100vh" }}
    >
      <Box
        display="flex"
        justifyContent="space-between"
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={8}>
          <SectionPaper
            ...
            onCancel={() => toggleEditSection("basic")}
          >
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={4}>
                <FormControl
                  fullWidth
                  size="small"
                  disabled={!editableData.country || !editSections.basic}
                >
              ...
              <Grid item xs={12} sm={4}>
                <FormControl
                  fullWidth
                  size="small"
                  disabled={!editableData.state || !editSections.basic}
                >
              ...
              <Grid item xs={12} sm={6}>
                <Box display="flex" alignItems="center" gap={1} height="100%">
                  ...
                  <Rating
                    ...
                    onChange={(_, val) => handleInputChange("starRating", val)}
          ...
          <SectionPaper
            ...
            onCancel={() => toggleEditSection("pricing")}
          >
            <Grid container spacing={2}>
              <Grid item xs={12} sm={4}>
                <TextField
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  type="date"
                  value={
                    editableData.from ? editableData.from.split("T")[0] : ""
                  }
          ...
          <SectionPaper
            ...
            onCancel={() => toggleEditSection("details")}
          >
            <Grid container spacing={2}>
              <Grid item xs={12}>
              ...
              <Grid item xs={12} md={6}>
                ...
                <TextField
                  ...
                  minRows={4}
                  size="small"
                  value={(editableData.inclusion || []).join("\n")}
                  onChange={(e) =>
                    handleInputChange("inclusion", e.target.value.split("\n"))
                  }
                  disabled={!editSections.details}
              ...
              <Grid item xs={12} md={6}>
                ...
                <TextField
                  ...
                  minRows={4}
                  size="small"
                  value={(editableData.exclusion || []).join("\n")}
                  onChange={(e) =>
                    handleInputChange("exclusion", e.target.value.split("\n"))
                  }
                  disabled={!editSections.details}
                  helperText="Enter each item on a new line."
                />
          ...
          <SectionPaper
            ...
            onCancel={() => toggleEditSection("terms")}
          >
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <Typography variant="subtitle2" fontWeight={500}>
          ...
          <SectionPaper
            ...
            onCancel={() => toggleEditSection("itinerary")}
          >
            <Grid container spacing={2}>
              {(editableData.dayWise || []).map((day, index) => (
                <Grid item xs={12} key={index}>
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      handleInputChange(
                        `dayWise.${index}.description`,
                        e.target.value
                      )
                    }
                    disabled={!editSections.itinerary}
                  />
                </Grid>
              ))}
