...
export default function MyComplaints() {
  ...
  // Ensure we fallback to empty array to prevent undefined errors
  const { filteredComplaints = [], loading, error } = useSelector((s) => s.complaints);
  ...
  const displayList = useMemo(() => {
    return filteredComplaints.filter((c) => {
      const matchId = !search || (c.complaintId || '').toLowerCase().includes(search.trim().toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 px-4 py-8 sm:px-6 lg:px-8 font-sans selection:bg-blue-100 selection:text-blue-900">
      <div className="mx-auto max-w-[1300px] space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm font-medium text-zinc-500">
              Track, manage, and follow up on your reported issues.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button 
              onClick={loadData} 
              disabled={loading}
              className="flex h-12 w-12 items-center justify-center rounded-2xl border border-zinc-200 bg-white text-zinc-600 shadow-sm transition-all hover:bg-zinc-50 hover:text-zinc-900 hover:shadow-md disabled:opacity-50"
              title="Refresh Data"
            >
        ...
        {filteredComplaints.length > 0 && (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            <div className="col-span-2 lg:col-span-1 rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm">
              ...
              <p className="text-3xl font-black text-zinc-900">{filteredComplaints.length}</p>
            </div>
            {[
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-2xl border-none bg-zinc-50/50 py-4 pl-12 pr-4 text-sm font-semibold text-zinc-800 outline-none transition-all focus:bg-zinc-100 placeholder:font-medium placeholder:text-zinc-400"
              />
            </div>
            ...
            <div className="flex items-center gap-2 px-2 sm:px-0">
              <div className="relative min-w-[160px]">
                ...
                <select
                  ...
                  className="w-full appearance-none rounded-2xl border-none bg-zinc-50/50 py-4 pl-11 pr-10 text-sm font-semibold text-zinc-800 outline-none transition-all focus:bg-zinc-100 cursor-pointer"
                >
              ...
              {(search || statusFilter) && (
                <button
                  onClick={() => { setSearch(''); setStatusFilter(''); dispatch(clearError()); }}
                  className="flex h-full items-center justify-center gap-2 rounded-2xl bg-rose-50 px-5 text-sm font-bold text-rose-600 transition-colors hover:bg-rose-100"
                >
        ...
        {/* Status Text (Bug fixed from userComplaints to filteredComplaints) */}
        {(search || statusFilter) && (
          <p className="px-2 text-xs font-bold uppercase tracking-widest text-blue-600">
            Showing {displayList.length} of {filteredComplaints.length} Result{filteredComplaints.length !== 1 ? 's' : ''}
          </p>
        )}
                ...
                {loading ? (
                      ...
                      <div className="flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
                        ...
                        <p className="mt-1 text-sm font-medium text-zinc-500 max-w-sm">
                          {search || statusFilter
                            ...
                            : "You currently have no active complaints or support tickets."}
                        </p>
                        {!search && !statusFilter && (
                          <button
                            onClick={() => navigate('/complaint/create')}
                  ...
                  displayList.map((c, idx) => (
                    <tr key={c._id || idx} className="group transition-colors hover:bg-zinc-50/80">
                      ...
                      <td className="px-6 py-5">
                        <p className="max-w-[220px] truncate text-sm font-medium text-zinc-700" title={c.issue}>
                          {c.issue || '—'}
                        </p>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => navigate(`/complaint/chat/${c._id}`)}
          ...
          {!loading && displayList.length > 0 && (
            <div className="border-t border-zinc-100 bg-zinc-50/50 px-6 py-4">
              <p className="text-xs font-bold uppercase tracking-widest text-zinc-400">
                Displaying {displayList.length} Ticket{displayList.length !== 1 ? 's' : ''}
              </p>
            </div>
          )}
        </div>
