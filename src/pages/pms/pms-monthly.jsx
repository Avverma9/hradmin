...
export default function PmsMonthly() {
  ...
  const normalizedHotels = useMemo(() => {
    const hotels = (allHotels || []).map(normalizeHotel);
    // Filter hotels by logged-in user's email
    return hotels.filter(hotel => {
      const ownerEmail = hotel.raw?.hotelOwnerEmail || hotel.raw?.email || hotel.email || '';
      return ownerEmail.toLowerCase() === user?.email?.toLowerCase();
    });
  }, [allHotels, user?.email]);
      ...
      <div className="mx-auto max-w-[1400px] space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <div className="mb-2 flex items-center gap-2">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-green-100 text-green-600"><BadgeDollarSign size={18} /></div>
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-green-600">PMS Panel</p>
                </div>
                <h1 className="text-3xl font-black tracking-tight text-gray-900 sm:text-4xl">Monthly Pricing</h1>
                <p className="mt-2 text-sm font-medium text-gray-500">Manage monthly pricing for your hotels.</p>
            </div>
            {selectedHotelId && (
                ...
                {selectedHotel && (
                    <div className="border-b border-gray-100 bg-gray-50/40 px-6 py-4 flex justify-between items-center flex-wrap gap-4">
                        <div className="flex flex-wrap items-center gap-3 text-sm font-semibold text-gray-500">
                         ...
                         <button onClick={() => setAddModal(true)} className="flex items-center gap-1 rounded-xl bg-green-600 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-green-600/20 transition-all hover:bg-green-700">
                ...
                {stats && (
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 p-6 bg-gray-50/30 border-b border-gray-100">
                        {[
                            ...
                            ['Avg Price', fmtCurrency(stats.avgPrice), 'text-green-700', 'bg-green-50', 'border-green-100'], 
                            ...
                            {loading ? (
                                ...
                                prices.map((p, idx) => (
                                    <tr key={p._id || idx} className="group transition-colors hover:bg-gray-50/80">
                                        ...
                                        <td className="px-6 py-5"><span className="text-base font-black text-green-700">{fmtCurrency(p.monthPrice)}</span></td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-1.5">
                                                <button onClick={() => setEditTarget(p)} title="Edit" className="flex h-9 w-9 items-center justify-center rounded-xl bg-green-50 text-green-600 transition-all hover:bg-green-600 hover:text-white hover:shadow-lg hover:shadow-green-600/20 active:scale-95"><Edit3 size={15} /></button>
        ...
        <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-xl shadow-gray-200/30">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-5">
                <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.2em] text-green-600">Your Hotels</p>
                    ...
                    <tbody className="divide-y divide-gray-100 bg-white">
                        {hotelsLoading ? (
                            ...
                            filteredHotels.map((hotel, idx) => { 
                                ...
                                return (
                                    <tr key={hotel.hotelId} onClick={() => { handleSelectHotel(hotel); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`cursor-pointer transition-colors hover:bg-gray-50/80 ${isSelected ? 'bg-green-50/70' : ''}`}>
                                        <td className="px-6 py-5"><span className="text-xs font-black text-gray-300">{String(idx + 1).padStart(2, '0')}</span></td>
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2.5">
                                                <div className={`rounded-xl p-2 ${isSelected ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}><Building2 size={15} /></div>
                                                <div>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex items-center gap-2">
                                                <button type="button" onClick={(e) => { e.stopPropagation(); handleSelectHotel(hotel); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className={`rounded-xl px-4 py-2.5 text-xs font-bold transition-all ${isSelected ? 'bg-green-600 text-white shadow-lg shadow-green-600/20' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
